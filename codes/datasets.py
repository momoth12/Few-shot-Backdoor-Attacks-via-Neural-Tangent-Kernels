...
MNIST_PATH = Path("./data/data_mnist")
...
MNIST_TRANSFORM_NORMALIZE = transforms.Normalize(
    MNIST_NORMALIZE_MEAN, MNIST_NORMALIZE_STD
)

# MNIST Transforms for training and testing
...
MNIST_TRANSFORM_TEST_XY = lambda xy: (MNIST_TRANSFORM_TEST(xy[0]), xy[1])
...
##################################################################################################################################
# Poisoner calls and some of the poisoner classes
##################################################################################################################################
...
def load_cifar_dataset(train=True): #Mouhamadou
    return datasets.CIFAR10(root=str(CIFAR_PATH), train=train, download=True)
    #dataset = datasets.CIFAR10(root=str(CIFAR_PATH), train=train, download=True)
    #if train:
    #    return Subset(dataset, list(range(1000)))  # smaller train dataset
    #else:
    #    return Subset(dataset, list(range(200)))   # smaller test dataset

def load_mnist_dataset(train=True):
    return datasets.MNIST(root=str(MNIST_PATH), train=train, download=True) 
...
def load_cifar_train(batch_size=32):
    ...
    #trainset = torch.utils.data.Subset(trainset, range(1000)) #mouhamadou
...
def load_cifar_test(batch_size=32):
    ...
    #testset = torch.utils.data.Subset(testset, range(200)) #mouhamadou
